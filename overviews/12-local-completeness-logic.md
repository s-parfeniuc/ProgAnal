...
command is $\forall P.\ A(\llbracket c\rrbracket P)=A(\llbracket c\rrbracket A(P))$ — abstracting
...
> $$A(\llbracket c\rrbracket P)\ =\ A(\llbracket c\rrbracket A(P)).$$
> Since always $A(\llbracket c\rrbracket P)\sqsubseteq A(\llbracket c\rrbracket A(P))$, this is
...
> $$A(Q)\ \supseteq\ \llbracket c\rrbracket P\ \supseteq\ Q.$$
> Two directions at once: **$\llbracket c\rrbracket P\supseteq Q$** — $Q$ **under-approximates**
> the reachable states (as in [IL](02-incorrectness-logic.md): every state of $Q$ is genuinely
> reachable); **$A(Q)\supseteq\llbracket c\rrbracket P$** — $A(Q)$ **over-approximates** them,
> and in fact $A(Q)=A(\llbracket c\rrbracket P)$ is a *sound abstract* post. So $Q$ is a
...
$$\dfrac{\mathcal C^A_P(e)}{\vdash_A [P]\ e\ [\llbracket e\rrbracket P]}\ [\textsf{transfer}]\qquad \text{(assignment: Floyd's post; test: the IL rule } \vdash_A[P]\,b?\,[P\wedge b]).$$
...
- **The payoff (why LCL is powerful).** If $\vdash_A [P]\,c\,[Q]$ then for an expressible spec:
  $$\llbracket c\rrbracket P\subseteq\mathit{Spec}\iff Q\subseteq\mathit{Spec}.$$
...
- **Soundness (logical correctness).** $\vdash_A [P]\,c\,[Q]\Rightarrow A(Q)=A(\llbracket c\rrbracket P)\wedge Q\subseteq\llbracket c\rrbracket P$. **Why:** each rule maintains the under/over sandwich; local completeness at every atomic step keeps $A(Q)$ equal to the abstract semantics.
...
### Local completeness
$$\mathcal C^A_P(c)\ \overset{\triangle}{\iff}\ A(\llbracket c\rrbracket P)=A(\llbracket c\rrbracket A(P))\quad(\text{always }\sqsubseteq;\ \text{equality required at }P\text{ only}).$$
...
### LCL triple (validity)
$$\vdash_A [P]\,c\,[Q]\ \text{valid}\iff A(Q)\supseteq\llbracket c\rrbracket P\supseteq Q\quad(\text{under-approx }Q\subseteq\llbracket c\rrbracket P,\ \text{and }A(Q)=A(\llbracket c\rrbracket P)).$$

### Rules ($\mathrm{LCL}_A$)
$$\dfrac{\mathcal C^A_P(e)}{\vdash_A [P]\,e\,[\llbracket e\rrbracket P]}\,[\textsf{transfer}]\quad \dfrac{\vdash_A [P]c_1[R]\ \ \vdash_A [R]c_2[Q]}{\vdash_A [P]c_1;c_2[Q]}\,[\textsf{seq}]\quad \dfrac{\vdash_A [P]c_1[Q_1]\ \ \vdash_A [P]c_2[Q_2]}{\vdash_A [P]c_1{+}c_2[Q_1\vee Q_2]}\,[\textsf{join}]$$
...
### Theorems
- **[Soundness]** $\vdash_A [P]\,c\,[Q]\Rightarrow Q\subseteq\llbracket c\rrbracket P\ \wedge\ A(Q)=A(\llbracket c\rrbracket P)$.
- **[Verification]** for expressible $\mathit{Spec}$: $\vdash_A [P]c[Q]\Rightarrow(\llbracket c\rrbracket P\subseteq\mathit{Spec}\iff Q\subseteq\mathit{Spec})$. $\;Q\not\subseteq\mathit{Spec}\Rightarrow$ true alert (no false alarm).
...
### The reconciliation
$$\underbrace{A(Q)\supseteq\llbracket c\rrbracket P}_{\text{over-approx (AI / HL)}}\ \wedge\ \underbrace{\llbracket c\rrbracket P\supseteq Q}_{\text{under-approx (IL)}}\ \Longrightarrow\ \text{prove correct }(Q\subseteq\mathit{Spec})\ \text{or find a true bug }(q\in Q\setminus\mathit{Spec}).$$
